...
(defvar my-keys-minor-mode-map
  (let ((map (make-sparse-keymap)))
    ...
    (define-key map (kbd "<Hangul>") 'hangul-input)
    map)
...
(defun ppt2pdf ()
  ...
  (shell-command (format "ppt2pdf --outdir %s %s*.pptx; rm %s*.pptx &" dir dir dir)))

(defun hangul-input ()
  "change input to hangul"
  (interactive)
  (lambda () (set-input-method 'korean-hangul2)))
